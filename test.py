...
couleur=[DARK_BLUE,YELLOW,WHITE,RED,BLUE,BLACK,GRAY]
...
class Planet:
    ...
    def draw(self):
        ...
        pygame.draw.circle(win, RED, (self.x, self.y), PLANET_SIZE)
...
class Spacecraft:
    ...
    def draw_lines(self):
        for i in self.passed_points:
            if i[2] % 2 == 0:
                pygame.draw.circle(win, random.choice(couleur), (i[0], i[1]), 1)
...
def main():
    ...
    planet = Planet(WIDTH // 2, HEIGHT // 4,0,0, PLANET_MASS)
    planet2 = Planet(WIDTH // 2, 900, 0, 0, PLANET_MASS)
    objects = []
    temp_obj_pos = None
    ...
    while running:
        ...
        win.fill(DARK_BLUE)  # Fond bleu foncé (galaxie)

        # Dessin des étoiles
        for star in stars:
            pygame.draw.circle(win, YELLOW, star, 1)  # Petits points jaunes
